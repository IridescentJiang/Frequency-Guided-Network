...
from datasets import teeth_no_plaque
...
def setup_loaders(args):
    ...
    elif args.dataset == 'teeth_no_plaque':
        args.dataset_cls = teeth_no_plaque
        args.train_batch_size = args.bs_mult * args.ngpu
        if args.bs_mult_val > 0:
            args.val_batch_size = args.bs_mult_val * args.ngpu
        else:
            args.val_batch_size = args.bs_mult * args.ngpu
    elif args.dataset == 'teeth_mask':
    ...
    mean_std_RGB = ([0.667, 0.602, 0.530], [0.168, 0.197, 0.189])
    mean_std_mask = ([0.689], [0.455])
    ...
    train_input_transform += [standard_transforms.ToTensor(),
                              standard_transforms.Normalize(*mean_std_RGB)]
    train_input_transform = standard_transforms.Compose(train_input_transform)

    train_input_mask_transfrom = []
    train_input_mask_transfrom += [standard_transforms.ToTensor(),
                                   standard_transforms.Normalize(*mean_std_mask)]
    train_input_mask_transfrom = standard_transforms.Compose(train_input_mask_transfrom)
    ...
    if args.dataset == 'cityscapes':
        ...
        if args.class_uniform_pct:
            if args.coarse_boost_classes:
                ...
                coarse_boost_classes = None
            train_set = args.dataset_cls.TeethMaskUniform(
                city_quality, city_mode, args.maxSkip,
                joint_transform_list=train_joint_transform_list,
                transform=train_input_transform,
                mask_transform=train_input_mask_transfrom,
                target_transform=target_train_transform,
                dump_images=args.dump_augmentation_images,
                cv_split=args.cv,
                class_uniform_pct=args.class_uniform_pct,
            ...
            train_set = args.dataset_cls.TeethMask(
                city_quality, city_mode, 0,
                joint_transform=train_joint_transform,
                transform=train_input_transform,
                mask_transform=train_input_mask_transfrom,
        ...
        val_set = args.dataset_cls.TeethMask('fine', 'val', 0,
                                             transform=val_input_transform,
                                             target_transform=target_transform,
                                             mask_transform=train_input_mask_transfrom,
                                             cv_split=args.cv)
