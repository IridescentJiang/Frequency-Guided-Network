...
import datasets.teeth_mask_labels as teeth_labels
import datasets.edge_utils as edge_utils

from config import cfg

trainid_to_name = teeth_labels.trainId2name
id_to_trainid = teeth_labels.label2trainid
num_classes = 3
ignore_label = 255
root = cfg.DATASET.TEETH_MASK_DIR
...
def add_items(items, aug_items, cities, img_path, mask_path, teeth_mask_path, mask_postfix, mode, maxSkip):
    """
    ...
    for c in cities:
        ...
        for it in c_items:
            ...
            if mode == "train" and maxSkip > 0:
                ...
                new_teeth_mask_path = os.path.join(root, 'mask_trainvaltest', 'mask')
...
def make_cv_splits(img_dir_name):
    ...
    split0 is aligned with the default teeth train/val.
    """
...
def make_dataset(quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0):
    ...
    if quality == 'coarse':
        ...
        teeth_mask_path = os.path.join(root, 'mask_trainvaltest', 'mask')
        mask_postfix = ''
        ...
        for mode in modes:
            if mode == 'test':
                ...
                add_items(items, aug_items, cv_splits, img_path, mask_path, teeth_mask_path,
                          mask_postfix, mode, maxSkip)
            else:
                logging.info('{} fine cities: '.format(mode) + str(cv_splits[cv_split][mode]))
                add_items(items, aug_items, cv_splits[cv_split][mode], img_path, mask_path, teeth_mask_path,
                          mask_postfix, mode, maxSkip)
    else:
        raise 'unknown teeth quality {}'.format(quality)

    logging.info('teeth-{}: {} images'.format(mode, len(items) + len(aug_items)))
...
class TeethMask(data.Dataset):

    def __init__(self, quality, mode, maxSkip=0, joint_transform=None, sliding_crop=None,
                 transform=None, mask_transform=None, target_transform=None, dump_images=False,
        ...
        self.mean_std = ([0.667, 0.602, 0.530, 0.673], [0.168, 0.197, 0.189, 0.461])

    def _eval_get_item(self, img, mask, teeth_mask, scales, flip_bool):
        return_imgs = []
        for flip in range(int(flip_bool) + 1):
            imgs = []
            if flip:
                img = img.transpose(Image.FLIP_LEFT_RIGHT)
                teeth_mask = teeth_mask.transpose(Image.FLIP_LEFT_RIGHT)
            for scale in scales:
                assert img.size == teeth_mask.size
                w, h = img.size
                target_w, target_h = int(w * scale), int(h * scale)
                resize_img = img.resize((target_w, target_h))
                resize_teeth_mask = teeth_mask.resize((target_w, target_h))
                tensor_img = transforms.ToTensor()(resize_img)
                tensor_teeth_mask = transforms.ToTensor()(resize_teeth_mask)
                tensor_teeth_mask = torch.unsqueeze(tensor_teeth_mask, 0)
                tensor_img = torch.cat((tensor_img, tensor_teeth_mask), dim=0)
    ...
    def __getitem__(self, index):
        ...
        img_path, mask_path, teeth_mask_path = self.imgs[index]

        img, mask, teeth_mask = Image.open(img_path).convert('RGB'), Image.open(mask_path), Image.open(teeth_mask_path)
        ...
        if self.eval_mode:
            return [transforms.ToTensor()(img)], self._eval_get_item(img, mask_copy, teeth_mask,
                                                                     self.eval_scales,
        ...
        if self.joint_transform is not None:
            img, mask, teeth_mask = self.joint_transform(img, mask, teeth_mask)
        if self.transform is not None:
            tensor_img = self.transform(img)
        if self.target_transform is not None:
            mask = self.target_transform(mask)
        if self.mask_transform is not None:
            tensor_teeth_mask = self.mask_transform(teeth_mask)

        #tensor_img = transforms.ToTensor()(img)
        #tensor_teeth_mask = transforms.ToTensor()(teeth_mask)
        tensor_img = torch.cat((tensor_img, tensor_teeth_mask), dim=0)
        #if self.mode != "test" and self.mode != "val":
        #    img = transforms.Normalize(*self.mean_std)(tensor_img)
        #else:
        #    img = tensor_img
        img = tensor_img
...
class TeethMaskUniform(data.Dataset):
    ...
    def __init__(self, quality, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        json_fn = 'teeth_{}_cv{}_tile{}.json'.format(
        ...
        if self.maxSkip > 0:
            json_fn = 'teeth_{}_cv{}_tile{}_skip{}.json'.format(
        ...
        if self.coarse_boost_classes is not None:
            json_fn = 'teeth_coarse_{}_tile{}.json'.format(
    ...
    def __getitem__(self, index):
        global teeth_mask_path
        elem = self.imgs_uniform[index]
        centroid = None
        if len(elem) == 5:
            img_path, mask_path, teeth_mask_path, centroid, class_id = elem
        elif len(elem) == 4:
            img_path, mask_path, centroid, class_id = elem
        else:
            img_path, mask_path, teeth_mask_path = elem
        ...
        if self.joint_transform_list is not None:
            for idx, xform in enumerate(self.joint_transform_list):
                if idx == 0 and centroid is not None:
                    ...
                    # in a centroid
                    img, mask, teeth_mask = xform(img, mask, teeth_mask, centroid)
                else:
                    img, mask, teeth_mask = xform(img, mask, teeth_mask)
